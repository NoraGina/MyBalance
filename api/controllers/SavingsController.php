...
class SavingsController extends AppController
{
    ...
    public function init()
    {
        ...
        if(isset($_SESSION['user']))
        {
            ...
            $year = date("Y");
            $month = date("M");
            $savings = $savingsModel-> getCurrentSavings($userId);
            if($savings)
            {
                
                $total = $savingsModel->getTotalSavingsCurrentMonth($userId);
                $data['mainContent'] = $this->displaySavings($savings, $loggedIn, $total, $year, $month);
            }else{
                $data['message']="$loggedIn nu ai economii introduse în $year $month <a href='addSavingsForm'> adugă una </a> sau <a href='searchSavingsForm'> Caută</a> altă dată";
                $data['mainContent']=$this->render(APP_PATH.VIEWS.'savingsErrorView.html', $data);
            }
