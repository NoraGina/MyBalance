    ...
    public function init()
    {
         ...
         if(isset($_SESSION['user']))
         {
            ...
            if(isset($_POST['submitForm']))
            {
                ...
                $selectedExpenses=$expensesModel->getCurrentExpensesByName($userId, $searchTermToLowerCase );
                if($selectedExpenses)
                {
                    $total = $expensesModel->getTotalExpensesCurrentMonthByName($userId, $searchTermToLowerCase );
                    $data['mainContent']=$this->displayExpenses($selectedExpenses, $loggedIn, $total);
                }else{
                    $data['error']= "$loggedIn Nu ai nicio cheltuială pentru $searchTerm luna aceasta";
                    $data['mainContent']=$this->render(APP_PATH.VIEWS.'errorsView.html', $data);
                }
            }elseif(isset($_POST['submitSelect']))
            {
                $category = $_POST['expenseCategory'];
                $expensOptions= $expensesModel->getCurrentExpensesByCategory($userId, $category);
                
                if($expensOptions)
                {
                    $total = $expensesModel->getTotalExpensesCurrentMonthByCategory($userId, $category);
                    $data['mainContent']=$this->displayExpenses($expensOptions, $loggedIn, $total);
                }else{
                    $data['error']= "$loggedIn Nu ai nicio cheltuială pentru $category luna aceasta";
                    $data['mainContent']=$this->render(APP_PATH.VIEWS.'errorsView.html', $data);
                }
            }else{
               
               $currentExpenses = $expensesModel->getCurrentExpenses($userId);
            
                if($currentExpenses)
                {
                    $total = $expensesModel->getTotalExpensesCurrentMonth($userId);
                    $data['mainContent']=$this->displayExpenses($currentExpenses, $loggedIn, $total);
                    
                }else{
                    $data['error']= "$loggedIn Nu ai nicio cheltuială introdusă luna aceasta";
     ...
     public function displayExpenses($expenses, $user, $total)
    {
        $output="";
        $expensesModel = new ExpensesModel;
        ...
        $output .='<h3 class="major" id="tableCaption">Toate cheltuielile tale  '. $user.' </h3>';
